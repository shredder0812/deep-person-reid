...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.sigmoid(avg_out + max_out).view(batch, channels, 1, 1, 1)
        return x * out
...
class SpatialAttention(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Conv3d(2, 1, kernel_size=7, stride=1, padding=3, bias=False)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        avg_out = torch.mean(x, dim=1, keepdim=True)  # Avg pooling trên kênh
        max_out, _ = torch.max(x, dim=1, keepdim=True)  # Max pooling trên kênh
        out = torch.cat([avg_out, max_out], dim=1)  # Ghép lại theo kênh
        out = self.conv(out)
...
class C3D(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pool5(x) # Output: (batch, 512, 1, 7, 7)
        ...
        x = x.squeeze(2)  # Loại bỏ chiều thời gian -> (batch, 512, 7, 7)
        return x
...
class C3D_OSNet(nn.Module):
    def __init__(self, num_classes=751):
        super(C3D_OSNet, self).__init__()
        self.c3d = C3D()
        self.osnet = OSNet(num_classes=num_classes)
    
    def forward(self, x):
        x = self.c3d(x)  # Trích xuất đặc trưng không gian-thời gian
        x = self.osnet(x).conv1(x)  # Nhận diện đối tượng từ đặc trưng
        x = self.osnet(x)
        return x
